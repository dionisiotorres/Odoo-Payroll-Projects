...
for reg in employee.payroll_contribution_reg_ids:
	if reg.contract_id and reg.contract_id.id != contract.id:
		continue
	if reg.type_id.code == 'ADV' and reg.valid_for_sal_rule(payslip.date_from, payslip.date_to, 15) and datet == 15:
                result = True
		break
...
if result:
    ...
    for reg in employee.payroll_contribution_reg_ids:
        if reg.contract_id and reg.contract_id.id != contract.id:
            continue
        if reg.type_id.code == 'ADV':
            payroll_contribution_reg = reg
            break
    if payroll_contribution_reg:
        result = payroll_contribution_reg.contribution_base/payroll_contribution_reg.x_period_of_contribution
